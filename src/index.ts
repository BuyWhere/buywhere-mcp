...
function formatProduct(p: Record<string, unknown>): string {
  ...
  const lines: string[] = [
    ...
    formatPriceLine(price, (p.source_url as string | undefined) ?? (p.url as string | undefined)),
    `Category: ${p.category ?? ""}`,
  ...
  if (name === "compare_products") {
    ...
    for (const p of products) {
      ...
      lines.push(
        ...
        formatPriceLine(price, (p.source_url as string | undefined) ?? (p.url as string | undefined)).replace(/^Price:/, "  Price:"),
        `  Category: ${p.category ?? ""}`,
