...
async function apiFetch(path, params, toolName = "unknown") {
...
async function apiPost(path, body, toolName = "unknown") {
...
function formatProduct(p) {
    ...
    const lines = [
        ...
        formatPriceLine(price, p.source_url ?? p.url),
        `Category: ${p.category ?? ""}`,
...
// MCP server
// ---------------------------------------------------------------------------
const server = new Server({ name: "buywhere-mcp", version: MCP_SERVER_VERSION }, {
    capabilities: {
        tools: {},
        resources: {},
    },
    ...
    tools: [
        {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    compact: {
                        type: "boolean",
                        description: "Return agent-optimized compact shape: structured_specs, comparison_attributes, normalized_price_usd. " +
                            "Reduces response size ~40%. Recommended for agent tool-use.",
                        default: false,
                    },
                    category: {
                        type: "string",
                        description: 'Filter by product category name (e.g. "Laptops", "Smartphones", "Televisions"). ' +
        ...
        },
        {
            ...
            description: 'Use this whenever a user asks about prices, wants to find the cheapest option, or asks "what\'s the best price for X" ' +
        ...
        },
        {
            ...
            description: "Ingest (upsert) a batch of products into the BuyWhere catalog. Use this to add or update product listings " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "search_products") {
        ...
        if (args.min_price !== undefined)
            params.min_price = Number(args.min_price);
        if (args.max_price !== undefined)
            params.max_price = Number(args.max_price);
        if (args.compact)
            params.compact = true;
        if (args.category)
    ...
    if (name === "compare_products") {
        const ids = (args.ids ?? args.product_ids);
        if (!ids || ids.length < 2) {
        ...
        const data = (await apiFetch("/v1/products/compare", { ids: ids.join(",") }, name));
        const products = data.products ?? [];
        const comparison = data.comparison;
        const lines = [`**Product comparison (${products.length} items):**\n`];
        for (const p of products) {
            const price = p.price;
            lines.push(`**${p.title ?? p.name}** (ID: ${p.product_id ?? p.id})`, formatPriceLine(price, p.source_url ?? p.url).replace(/^Price:/, "  Price:"), `  Category: ${p.category ?? ""}`, `  Merchant: ${p.merchant?.name ?? ""}`, `  URL: ${p.source_url ?? p.url ?? ""}`, "");
        }
        if (comparison) {
    ...
    if (name === "get_deals") {
        ...
        if (args.min_discount !== undefined)
            params.min_discount = Number(args.min_discount);
        if (args.currency)
        ...
        if (args.offset)
            params.offset = Number(args.offset);
        const data = (await apiFetch("/v1/deals", params, name));
        const results = (data.results ?? data.deals ?? data);
        const items = Array.isArray(results) ? results : [];
        if (items.length === 0) {
            return { content: [{ type: "text", text: "No deals found matching the given criteria." }] };
        }
        const lines = [`**${items.length} deal(s) found:**\n`];
        for (const item of items) {
            const price = item.price;
            const originalPrice = item.original_price;
            const discount = item.discount_percentage ?? item.discount_pct ?? "";
            lines.push(`**${item.title ?? item.name}**`, `  Price: ${price?.currency ?? "SGD"} ${price?.amount ?? price?.total ?? "N/A"}` +
                (originalPrice ? ` (was ${originalPrice.currency ?? "SGD"} ${originalPrice.amount})` : ""), discount ? `  Discount: ${discount}%` : "", `  URL: ${item.source_url ?? item.url ?? ""}`, "");
        }
    ...
    if (name === "ingest_products") {
        ...
        const data = (await apiPost("/v1/products/ingest", { source, products }, name));
        return {
            content: [
                {
                    type: "text",
